...
function renderClose(tokens, idx) {
    // Walk backwards to find the matching open token, counting nesting depth
    // so nested admonitions don't confuse the matching.
    let depth = 0;
    let openIdx = idx - 1;
    while (openIdx >= 0) {
        if (tokens[openIdx].type === 'admonition_close') {
            depth++;
        } else if (tokens[openIdx].type === 'admonition_open') {
            if (depth === 0) break;
            depth--;
        }
        openIdx--;
