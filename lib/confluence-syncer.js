...
async function sync() {
    try {
        ...
        const pageMapping = await syncPages(home, localPages, sectionHierarchy, assetRenderer);
...
async function syncPages(home, localPages, sectionHierarchy, renderer) {
    // Collected page ID → path mappings for comment-sync
    const pageMapping = [];
    ...
    const processLevel = async (sectionName) => {
        ...
        for (let i = 0; i < union.length; i++) {
            ...
            // Collect mapping for synced pages (skip deleted pages)
            if (syncedPage && syncedPage.id && syncedPage.meta?.path) {
                pageMapping.push({
                    pageId: syncedPage.id,
                    pagePath: syncedPage.meta.path,
...
/**
 * Write page-mapping.json and set action output.
 *
 * @param {Array<{pageId: number, pagePath: string, title: string}>} pageMapping
 */
function emitPageMapping(pageMapping) {
    const mappingPath = 'page-mapping.json';
    writeFileSync(mappingPath, JSON.stringify(pageMapping, null, 2));
    setOutput('page_mapping_file', mappingPath);
    setOutput('page_mapping', JSON.stringify(pageMapping));
    logger.info(`Page mapping written to ${mappingPath} (${pageMapping.length} pages)`);
}
