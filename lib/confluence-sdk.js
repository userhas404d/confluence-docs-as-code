...
class ConfluenceSdk {
    ...
    async getPageChildren(pageId) {
        ...
        while (nextUri) {
            ...
            const pages = data.results.filter(item => item.type === 'page');
            children.push(...pages);
    ...
    async downloadAttachment(downloadUrl, destPath) {
        ...
        if (response.status !== 200) {
            throw new RequestError(response.status, response.statusText, 'Download failed');
        }
