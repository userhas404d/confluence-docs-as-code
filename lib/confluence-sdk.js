...
class ConfluenceSdk {
    ...
    // ── Comment Methods (v1 API) ────────────────────────────────────

    /**
     * Fetch all comments for a Confluence page.
     * Paginates automatically.
     *
     * @param {number} pageId - Confluence page ID
     * @returns {Promise<Array<object>>} Array of comment objects with body.storage and version
     */
    async getComments(pageId) {
        const comments = [];
        const expand = 'body.storage,version,extensions.inlineProperties';
        let nextUri = `${CONTENT_PATH}/${pageId}/child/comment?expand=${expand}&depth=all&limit=25`;
        ...
        while (nextUri) {
            ...
            nextUri = data._links?.next
                ? data._links.context + data._links.next
                : null;
        }
     ...
     *
     * @param {number} pageId - Confluence page ID
     * @param {string} body - Comment body in Confluence storage format (HTML)
     ...
     * @param {number} version - The current version number (will be incremented)
     * @param {string} body - Updated comment body in Confluence storage format
     * @returns {Promise<object>} The updated comment object
     */
    async updateComment(commentId, version, body) {
        const payload = {
            type: 'comment',
